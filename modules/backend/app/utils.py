...
def calculate_multimodal_risk_100_scale(nlp_raw_score: int, yolo_raw_score: int):
    ...
    """
    # 排序分數＝文字分數本身，不再是加權平均。
    # 加權實測比純文字差很多（ROC-AUC 0.847 vs 0.900）：影像單獨只有 0.53，
    # 混進去等於在好訊號裡摻雜訊，權重越高越差。
    # 影像改用在「同一級之內誰先看」，那是 ORDER BY 的次要鍵（見 crawler.py）。
...
def dispatch_to_ai_engines(url: str, html_content: str, images: list):
    ...
    #
    # 沒有圖時要明確寫「無影像可分析」，不能留在「影像分析中...」。
    # 那個字串是各處判斷「還在跑」的依據（scan.py 決定要不要重派、前端決定要不要
    # 繼續輪詢），沒有商品圖時它永遠不會被覆蓋，畫面就一直轉。
    if not images:
...
# OCR 文字要跟網頁文字「合併」後再判一次，不是單獨送。
# 單獨送的話模型拿到一袋沒有上下文的碎片，會把普通商品頁判成高分，
# 而且「分數較高才覆蓋」會把網頁文字算出的關鍵字整組蓋掉。
# 兩者本來就是同一頁的內容，一起判才有上下文，也只會有一組答案。
