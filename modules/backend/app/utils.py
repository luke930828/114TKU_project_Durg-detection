...
# 這是「要不要進覆核清單」的下限，低於它就完全不會被人看到。
#
# 門檻必須跟著模型版本調。舊模型的分數幾乎只有 0 或 100，60 放在中間沒差；
# v3 的分布不同——正樣本有幾筆落在 6~87 之間，用 60 會把它們判成低風險而漏掉。
# 48 筆 held-out 實測：MED=60 漏 4 個、MED=5 漏 2 個（與舊模型相同），
# 而誤判從 9 降到 3。1~5 之間結果完全一樣，取 5 留一點餘裕。
NLP_MEDIUM = 5
