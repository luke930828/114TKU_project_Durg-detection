...
# 這兩個門檻是對「校準後的分數」設的（見 nlp/app/calibration.py）。
# 校準讓分數真的代表機率：90-100 分那批實際有 94.7% 是毒品站、
# 50-69 分那批是 50.0%。所以門檻可以照字面理解，不必記「5 分其實很高」。
#
# 校準前後是等價的，只是尺度不同——實測命中率完全相同：
#     校準前 HIGH=90 / MEDIUM=5   極高 95.2%、高 90.9%、中 57.1%
#     校準後 HIGH=85 / MEDIUM=30  極高 95.2%、高 90.9%、中 57.1%
#
# 換模型時這三個值和校準參數要一起重新量，它們是綁在一起的。
