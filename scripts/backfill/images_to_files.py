...
為什麼要搬
──────────
這個欄位佔了 10.4 GB，是整個資料庫的八成，而 MySQL 的 buffer pool 放不下，
所有查詢都在打磁碟（命中率長期只有 38%）。圖片本身是蒐證資料，不能刪，
所以是換地方存：檔案放 SUSPECT_IMAGE_DIR，欄位改存路徑。
搬完資料庫會從 12.7 GB 掉到 2.3 GB 左右。

安全性
──────
一筆一筆處理，而且是「先把檔案寫好、確認讀得回來、才改欄位」。
中途中斷不會壞：已經搬好的欄位是路徑，還沒搬的仍是 base64，
兩種格式 image_store.load_field() 都吃，補跑腳本照樣能用。
重跑會自動跳過已經搬好的，可以放心續跑。
